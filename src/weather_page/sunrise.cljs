; Borrowed & ported to ClojureScript from https://github.com/mrwhelan/sunrise

(ns weather-page.sunrise)
...
(defn- d->r [d] (/ (* d Math/PI) 180))
(defn- r->d [r] (/ (* r 180) Math/PI))

;; We're calculating in "degree mode", so we'll use these functions
(defn- sin [x] (Math/sin (d->r x)))
(defn- cos [x] (Math/cos (d->r x)))
(defn- tan [x] (Math/tan (d->r x)))
(defn- asin [x] (r->d (Math/asin x)))
(defn- acos [x] (r->d (Math/acos x)))
(defn- atan [x] (r->d (Math/atan x)))
